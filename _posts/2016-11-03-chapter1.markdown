...
그렇다면 의존관계 검색과 의존관계 주입 중 어떤 것이 더 나을까? 코드를 보면 느낄 수 있겠지만 의존관계 주입 쪽이 훨씬 단순하고 깔끔하다. 그런데 의존관계 검색을 사용해야 할 때가 있다. 앞에서 만들었던 테스트 코드인 UserDaoTest를 보자. 테스트 코드에서는 이미 의존관계 검색 방식인 getBean()을 사용했다. 스태틱 메소드인 main()에서는 DI를 이용해 오브젝트를 주입받을 방법이 없기 때문이다.
